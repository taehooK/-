...
public:
	...
	Array<Array<T>> ConvertTo2D(Long columnCount);
	...
	Long Max(int(*compare)(void*, void*));
	Long Min(int(*compare)(void*, void*));
...
template<typename T>
void Array<T>::Resize(Long capacity) {
	T(*objects);
	Long i = 0;
...
template<typename T>
Array<Array<T>> Array<T>::ConvertTo2D(Long columnCount) {
	assert(this->length % columnCount == 0);
	Long capacity = this->length / columnCount;
	
	Array<Array<T>> matrix(capacity);

	Long k = 0;
	Long i = 0;
	while (i < capacity)
	{
		Array<T> temp(columnCount);
		Long j = 0;
		while (j < columnCount)
		{
			temp.Store(j, this->front[k++]);
			j++;
		}
		matrix.Store(i, temp);
...
template<typename T>
Long  Array<T>::Max(int(*compare)(void*, void*)) {
	Long maxIndex = -1;
	Long i = 0;
	T max;

	while (i < this->length) {
		if (i == 0 || compare(&max, &this->front[i]) < 0) {
			maxIndex = i;
			max = this->front[maxIndex];
		}
		i++;
	}
	return maxIndex;
...
template<typename T>
Long  Array<T>::Min(int(*compare)(void*, void*)) {
	Long minIndex = -1;
	Long i = 0;
	T min;

	while (i < this->length) {
		if (i == 0 || compare(&min, &this->front[i]) > 0) {
			minIndex = i;
			min = this->front[minIndex];
		}
		i++;
